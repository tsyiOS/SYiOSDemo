...
static void print_report(int is_last_report, int64_t timer_start, int64_t cur_time ,pcb cb)
{
    char buf[1024];
    AVBPrint buf_script;
    OutputStream *ost;
    ...
    if (total_size < 0) snprintf(buf + strlen(buf), sizeof(buf) - strlen(buf),
                                 ...
                                 "size=%8.0fkB time22=", total_size / 1024.0);
    if (pts < 0)
...
static int transcode(pcb cb)
{
    ...
    while (!received_sigterm) {
        ...
        print_report(0, timer_start, cur_time,cb);
    }
#if HAVE_PTHREADS
    free_input_threads();
#endif
    ...
    print_report(1, timer_start, av_gettime_relative(),cb);
...
int ffmpeg_main(int argc, char **argv,pcb cb)
{
    int ret;
    int64_t ti;
